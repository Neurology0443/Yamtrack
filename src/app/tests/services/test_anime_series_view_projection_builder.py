...
class AnimeSeriesViewProjectionBuilderTests(SimpleTestCase):
    ...
    def test_alternative_branch_context_propagates_to_branch_continuation(self):
        projection = self.builder.build(
            snapshot=snapshot(
                [
    ...
    def test_alternative_branch_context_survives_continuation_only_filter(self):
        projection = self.builder.build(
            snapshot=snapshot(
                [
                    node("1", title="Main TV"),
        ...
        branch = projection.groups[0]
        self.assertEqual(branch.root_media_id, "2")
        self.assertEqual(branch.member_media_ids, ("3",))
        self.assertEqual(branch.group_kind, "alternative_branch")
        self.assertEqual(branch.context_parent_media_id, "1")
        self.assertEqual(branch.context_parent_title, "Main TV")
        self.assertEqual(branch.context_relation_type, "alternative_version")

    def test_spin_off_context_propagates_to_branch_continuation(self):
        projection = self.builder.build(
            snapshot=snapshot(
                [
                    node("1", title="Main TV"),
                    node("2", title="Spin-off 1"),
                    node("3", title="Spin-off 2"),
                ],
                [
                    relation("1", "2", "spin_off"),
                    relation("2", "3", "sequel"),
    ...
    def test_branch_context_does_not_cross_another_strong_boundary(self):
        projection = self.builder.build(
            snapshot=snapshot(
                [
                    node("1", title="Main TV"),
                    node("2", title="Alternative Branch"),
                    node("3", title="Nested Spin-off"),
                ],
                [
                    relation("1", "2", "alternative_version"),
                    relation("2", "3", "spin_off"),
                ],
                series_line_ids=["1"],
            ),
            tracked_media_ids={"2", "3"},
        ...
        nested = next(
            group for group in projection.groups if group.root_media_id == "3"
        )
        self.assertEqual(alternative.group_kind, "alternative_branch")
        self.assertEqual(alternative.context_parent_media_id, "1")
        self.assertIsNone(nested.context_parent_media_id)
        self.assertIsNone(nested.context_relation_type)
